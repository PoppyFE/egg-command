...
class StartCommand extends Command {
  constructor(rawArgv) {
    super(rawArgv);
    this.usage = 'Usage: egg-command start [options] [baseDir]';
    ...
    this.options = {
      ...
      node: {
        description: 'specify node to run the app',
        type: 'string',
      },
      daemon: {
  ...
  * run(context) {
    ...
    const HOME = homedir();
    const logDir = env.EGG_LOG_DIR || path.join(HOME, 'logs');

    // egg-command start
    // egg-command start ./server
    // egg-command start /opt/app
    ...
    let nodeBin = 'node';
    if (argv.node) {
      if(argv.node.charAt(0) === '~') {
        argv.node = os.homedir() + argv.node.substring(1);
      }
      nodeBin = argv.node;
      this.logger.info('Use specified node path s%', nodeBin);
    } else {
      this.logger.info('Use default system node path');
    }
    ...
    if (isDaemon) {
      ...
      const child = this.child = spawn(nodeBin, eggArgs, options);
      this.isReady = false;
      ...
      this.helper.spawn(nodeBin, eggArgs, options);
    }
  }
